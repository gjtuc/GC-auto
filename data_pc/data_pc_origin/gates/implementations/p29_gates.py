# -*- coding: utf-8
"""P29 L4 gate bodies — GitHub refresh (P27–P28)."""
...
from data_pc_origin.gates.registry import P29_DEPS, register_gate
...
def _gate_p29_g_01_a_1() -> None:
    plan = plan_github_refresh_post28(_script_dir())
    _assert(plan.snapshot_ready, plan.reason)
    _assert(plan.gate_count >= 206)
    _assert(plan.branch == SNAPSHOT_BRANCH)


def _gate_p29_g_02_a_1() -> None:
    plan = plan_github_refresh_post28(_script_dir())
    _assert(plan.markers_ready)
    _assert(all(m.source_ok for m in plan.markers))


def _gate_p29_g_03_a_1() -> None:
...
def _gate_p29_g_04_a_1() -> None:
    _assert(github_push_enabled({GITHUB_PUSH_ENV: "1"}))
    _assert(not github_push_enabled({GITHUB_PUSH_ENV: "0"}))


def _gate_p29_h_01_a_1() -> None:
    root = Path(__file__).resolve().parents[2]
    out = run_live_p29_github_refresh(artifact_dir=root, script_dir=_script_dir())
    _assert(out["status"] in ("ok", "partial"))
    _assert(out.get("artifact_valid") is True)


def _gate_p29_h_02_a_1() -> None:
    root = Path(__file__).resolve().parents[2]
    _assert((root / ARTIFACT_NAME).is_file())


def _gate_p29_h_03_a_1() -> None:
    saved = os.environ.pop(GITHUB_PUSH_ENV, None)
    try:
        out = run_live_p29_github_refresh(script_dir=_script_dir(), do_push=True)
...
def _gate_p29_h_04_a_1() -> None:
    root = Path(__file__).resolve().parents[2]
    out = run_live_p29_github_refresh(artifact_dir=root, script_dir=_script_dir())
    _assert(validate_github_refresh_post28_artifact(out))
    _assert(out["plan"]["markers_ready"] is True)


_P29_GATES: list[tuple[str, object]] = [
    ("P29-G-01-a-1", _gate_p29_g_01_a_1),
    ("P29-G-02-a-1", _gate_p29_g_02_a_1),
    ("P29-G-03-a-1", _gate_p29_g_03_a_1),
    ("P29-G-04-a-1", _gate_p29_g_04_a_1),
    ("P29-H-01-a-1", _gate_p29_h_01_a_1),
    ("P29-H-02-a-1", _gate_p29_h_02_a_1),
    ("P29-H-03-a-1", _gate_p29_h_03_a_1),
    ("P29-H-04-a-1", _gate_p29_h_04_a_1),
]


def register_p29_gates() -> None:
    for gate_id, fn in _P29_GATES:
        register_gate(gate_id, fn, depends=P29_DEPS[gate_id], layer="P29")  # type: ignore[arg-type]
