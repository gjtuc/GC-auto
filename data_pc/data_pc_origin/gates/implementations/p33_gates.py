# -*- coding: utf-8
"""P33 L4 gate bodies — GitHub push."""
...
from data_pc_origin.gates.registry import P33_DEPS, register_gate
...
def _gate_p33_g_04_a_1() -> None:
    plan = plan_github_push_post32(_script_dir())
    _assert(plan.snapshot_ready)


def _gate_p33_h_01_a_1() -> None:
    root = Path(__file__).resolve().parents[2]
    out = run_live_p33_github_push(artifact_dir=root, script_dir=_script_dir())
    _assert(out["status"] in ("ok", "partial"))
    _assert(out.get("artifact_valid") is True)


def _gate_p33_h_02_a_1() -> None:
    root = Path(__file__).resolve().parents[2]
    _assert((root / ARTIFACT_NAME).is_file())


def _gate_p33_h_03_a_1() -> None:
    saved = os.environ.pop(GITHUB_PUSH_ENV, None)
    try:
        out = run_live_p33_github_push(script_dir=_script_dir(), do_push=True)
...
def _gate_p33_h_04_a_1() -> None:
    root = Path(__file__).resolve().parents[2]
    out = run_live_p33_github_push(artifact_dir=root, script_dir=_script_dir())
    _assert(validate_github_push_post32_artifact(out))
    _assert(out["plan"]["dest_markers_ok"] is True)


_P33_GATES: list[tuple[str, object]] = [
    ("P33-G-01-a-1", _gate_p33_g_01_a_1),
    ("P33-G-02-a-1", _gate_p33_g_02_a_1),
    ("P33-G-03-a-1", _gate_p33_g_03_a_1),
    ("P33-G-04-a-1", _gate_p33_g_04_a_1),
    ("P33-H-01-a-1", _gate_p33_h_01_a_1),
    ("P33-H-02-a-1", _gate_p33_h_02_a_1),
    ("P33-H-03-a-1", _gate_p33_h_03_a_1),
    ("P33-H-04-a-1", _gate_p33_h_04_a_1),
]


def register_p33_gates() -> None:
    for gate_id, fn in _P33_GATES:
        register_gate(gate_id, fn, depends=P33_DEPS[gate_id], layer="P33")  # type: ignore[arg-type]
