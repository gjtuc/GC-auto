# -*- coding: utf-8
"""P35 L4 gate bodies — GitHub push."""
...
from data_pc_origin.gates.registry import P35_DEPS, register_gate
...
def _gate_p35_g_04_a_1() -> None:
    plan = plan_github_push_post34(_script_dir())
    _assert(plan.snapshot_ready)


def _gate_p35_h_01_a_1() -> None:
    root = Path(__file__).resolve().parents[2]
    out = run_live_p35_github_push(artifact_dir=root, script_dir=_script_dir())
    _assert(out["status"] in ("ok", "partial"))
    _assert(out.get("artifact_valid") is True)


def _gate_p35_h_02_a_1() -> None:
    root = Path(__file__).resolve().parents[2]
    _assert((root / ARTIFACT_NAME).is_file())


def _gate_p35_h_03_a_1() -> None:
    saved = os.environ.pop(GITHUB_PUSH_ENV, None)
    try:
        out = run_live_p35_github_push(script_dir=_script_dir(), do_push=True)
...
def _gate_p35_h_04_a_1() -> None:
    root = Path(__file__).resolve().parents[2]
    out = run_live_p35_github_push(artifact_dir=root, script_dir=_script_dir())
    _assert(validate_github_push_post34_artifact(out))
    _assert(out["plan"]["dest_markers_ok"] is True)


_P35_GATES: list[tuple[str, object]] = [
    ("P35-G-01-a-1", _gate_p35_g_01_a_1),
    ("P35-G-02-a-1", _gate_p35_g_02_a_1),
    ("P35-G-03-a-1", _gate_p35_g_03_a_1),
    ("P35-G-04-a-1", _gate_p35_g_04_a_1),
    ("P35-H-01-a-1", _gate_p35_h_01_a_1),
    ("P35-H-02-a-1", _gate_p35_h_02_a_1),
    ("P35-H-03-a-1", _gate_p35_h_03_a_1),
    ("P35-H-04-a-1", _gate_p35_h_04_a_1),
]


def register_p35_gates() -> None:
    for gate_id, fn in _P35_GATES:
        register_gate(gate_id, fn, depends=P35_DEPS[gate_id], layer="P35")  # type: ignore[arg-type]
