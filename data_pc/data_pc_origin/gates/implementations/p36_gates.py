# -*- coding: utf-8
"""P36 L4 gate bodies — GitHub refresh (P34–P35)."""
...
from data_pc_origin.gates.registry import P36_DEPS, register_gate
from data_pc_origin.live_p36_github_refresh import ARTIFACT_NAME, run_live_p36_github_refresh
from data_pc_origin.p23_github_snapshot import GITHUB_PUSH_ENV, SNAPSHOT_BRANCH, github_push_enabled
from data_pc_origin.p36_github_refresh import (
    plan_github_refresh_post35,
    sync_github_refresh_post35,
    validate_github_refresh_post35_artifact,
    verify_dest_markers_post35,
)
...
def _gate_p36_g_01_a_1() -> None:
    plan = plan_github_refresh_post35(_script_dir())
    _assert(plan.snapshot_ready, plan.reason)
    _assert(plan.gate_count >= 262)
    _assert(plan.branch == SNAPSHOT_BRANCH)


def _gate_p36_g_02_a_1() -> None:
    plan = plan_github_refresh_post35(_script_dir())
    _assert(plan.markers_ready)
    _assert(all(m.source_ok for m in plan.markers))


def _gate_p36_g_03_a_1() -> None:
...
def _gate_p36_g_04_a_1() -> None:
    _assert(github_push_enabled({GITHUB_PUSH_ENV: "1"}))
    _assert(not github_push_enabled({GITHUB_PUSH_ENV: "0"}))


def _gate_p36_h_01_a_1() -> None:
    root = Path(__file__).resolve().parents[2]
    out = run_live_p36_github_refresh(artifact_dir=root, script_dir=_script_dir())
    _assert(out["status"] in ("ok", "partial"))
    _assert(out.get("artifact_valid") is True)


def _gate_p36_h_02_a_1() -> None:
    root = Path(__file__).resolve().parents[2]
    _assert((root / ARTIFACT_NAME).is_file())


def _gate_p36_h_03_a_1() -> None:
    saved = os.environ.pop(GITHUB_PUSH_ENV, None)
    try:
        out = run_live_p36_github_refresh(script_dir=_script_dir(), do_push=True)
...
def _gate_p36_h_04_a_1() -> None:
    root = Path(__file__).resolve().parents[2]
    out = run_live_p36_github_refresh(artifact_dir=root, script_dir=_script_dir())
    _assert(validate_github_refresh_post35_artifact(out))
    _assert(out["plan"]["markers_ready"] is True)


_P36_GATES: list[tuple[str, object]] = [
    ("P36-G-01-a-1", _gate_p36_g_01_a_1),
    ("P36-G-02-a-1", _gate_p36_g_02_a_1),
    ("P36-G-03-a-1", _gate_p36_g_03_a_1),
    ("P36-G-04-a-1", _gate_p36_g_04_a_1),
    ("P36-H-01-a-1", _gate_p36_h_01_a_1),
    ("P36-H-02-a-1", _gate_p36_h_02_a_1),
    ("P36-H-03-a-1", _gate_p36_h_03_a_1),
    ("P36-H-04-a-1", _gate_p36_h_04_a_1),
]


def register_p36_gates() -> None:
    for gate_id, fn in _P36_GATES:
        register_gate(gate_id, fn, depends=P36_DEPS[gate_id], layer="P36")  # type: ignore[arg-type]
