# -*- coding: utf-8
"""P29 — GitHub refresh harness (P27–P28 markers)."""
...
ARTIFACT_NAME = "live_p29_github_refresh_result.json"


def run_live_p29_github_refresh(
    *,
    ...
    """P29 실행 검증 — plan / sync / push (P27–P28 markers)."""
    root_dir = script_dir or str(Path(__file__).resolve().parent.parent)
    plan = plan_github_refresh_post28(root_dir)
    ...
    if do_sync:
        sync = sync_github_refresh_post28(root_dir, dry_run=False)
        out["sync"] = sync
        out["mode"] = "sync"
    ...
    if do_push:
        if not github_push_enabled():
            ...
            result = push_github_refresh_post28(root_dir)
            out = {
    ...
    out["artifact_valid"] = validate_github_refresh_post28_artifact(out)
...
def main() -> int:
    ...
    result = run_live_p29_github_refresh(do_sync=do_sync, do_push=do_push)
