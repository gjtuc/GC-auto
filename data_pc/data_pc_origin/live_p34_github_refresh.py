# -*- coding: utf-8
"""P34 — GitHub refresh harness (P32–P33 markers)."""
...
ARTIFACT_NAME = "live_p34_github_refresh_result.json"


def run_live_p34_github_refresh(
    *,
    ...
    """P34 실행 검증 — plan / sync / push (P32–P33 markers)."""
    root_dir = script_dir or str(Path(__file__).resolve().parent.parent)
    plan = plan_github_refresh_post33(root_dir)
    ...
    if do_sync:
        sync = sync_github_refresh_post33(root_dir, dry_run=False)
        out["sync"] = sync
        out["mode"] = "sync"
    ...
    if do_push:
        if not github_push_enabled():
            ...
            result = push_github_refresh_post33(root_dir)
            out = {
    ...
    out["artifact_valid"] = validate_github_refresh_post33_artifact(out)
...
def main() -> int:
    ...
    result = run_live_p34_github_refresh(do_sync=do_sync, do_push=do_push)
