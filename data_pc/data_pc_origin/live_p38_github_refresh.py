# -*- coding: utf-8
"""P38 — GitHub refresh harness (P36–P37 markers)."""
...
ARTIFACT_NAME = "live_p38_github_refresh_result.json"


def run_live_p38_github_refresh(
    *,
    ...
    """P38 실행 검증 — plan / sync / push (P36–P37 markers)."""
    root_dir = script_dir or str(Path(__file__).resolve().parent.parent)
    plan = plan_github_refresh_post37(root_dir)
    ...
    if do_sync:
        sync = sync_github_refresh_post37(root_dir, dry_run=False)
        out["sync"] = sync
        out["mode"] = "sync"
    ...
    if do_push:
        if not github_push_enabled():
            ...
            result = push_github_refresh_post37(root_dir)
            out = {
    ...
    out["artifact_valid"] = validate_github_refresh_post37_artifact(out)
...
def main() -> int:
    ...
    result = run_live_p38_github_refresh(do_sync=do_sync, do_push=do_push)
