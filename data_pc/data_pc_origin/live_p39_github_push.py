...
ARTIFACT_NAME = "live_p39_github_push_result.json"


def run_live_p39_github_push(
    *,
    artifact_dir: Optional[Path] = None,
    script_dir: Optional[str] = None,
    do_push: bool = False,
) -> Dict[str, object]:
    """P39 실행 검증 — push plan (+ optional gated push)."""
    root_dir = script_dir or str(Path(__file__).resolve().parent.parent)
    plan = plan_github_push_post38(root_dir)
    ...
    if do_push:
        if not github_push_enabled():
            ...
            result = push_github_post38(root_dir)
            out = {
    ...
    out["artifact_valid"] = validate_github_push_post38_artifact(out)
...
def main() -> int:
    ...
    result = run_live_p39_github_push(do_push=do_push)
