# -*- coding: utf-8
"""P29 — GitHub refresh sync (P27–P28 → feat/data-pc-origin)."""
...
REFRESH_MARKER_DOCS: tuple[str, ...] = (
    "design/catalog/P27.md",
    "design/catalog/P28.md",
)
...
def plan_github_refresh_post28(script_dir: str) -> GithubRefreshPlan:
    """P23 snapshot + P27–P28 marker readiness."""
...
def verify_dest_markers_post28(script_dir: str) -> Dict[str, Any]:
    markers = _collect_markers(script_dir)
    return {
...
def sync_github_refresh_post28(script_dir: str, *, dry_run: bool = False) -> Dict[str, Any]:
    ...
    dest_markers = verify_dest_markers_post28(script_dir) if not dry_run else {"ok": True, "markers": []}
    return {
        "status": "ok",
        "dry_run": dry_run,
        "sync": sync,
...
def push_github_refresh_post28(
    script_dir: str,
    *,
    message: Optional[str] = None,
) -> Dict[str, Any]:
    sync = sync_github_refresh_post28(script_dir, dry_run=False)
    if sync["status"] != "ok":
        return {"status": "error", "stage": "sync", "sync": sync}

    msg = message or f"chore(data-pc): P29 github refresh ({len(P28_EXTENDED_ORDER)} gates)"
    push = push_snapshot(script_dir, message=msg)
    push["refresh"] = sync
    push["dest_markers"] = verify_dest_markers_post28(script_dir)
    return push


def validate_github_refresh_post28_artifact(payload: Dict[str, Any]) -> bool:
    if payload.get("status") not in ("ok", "skipped", "partial"):
        return False
    plan = payload.get("plan")
    if not isinstance(plan, dict):
        return False
    return plan.get("gate_count", 0) >= 206 and "markers" in plan
