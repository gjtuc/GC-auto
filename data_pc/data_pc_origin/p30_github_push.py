# -*- coding: utf-8
"""P30 — GitHub push (P29 sync → feat/data-pc-origin remote)."""
...
from data_pc_origin.gates.registry import P29_EXTENDED_ORDER
...
def plan_github_push_post29(script_dir: str) -> GithubPushPlan:
    """P29 dest markers + git branch state."""
    ...
    return GithubPushPlan(
        ...
        gate_count=len(P29_EXTENDED_ORDER),
...
def push_github_post29(
    script_dir: str,
    *,
    message: Optional[str] = None,
) -> Dict[str, Any]:
    """`DATA_PC_GITHUB_PUSH=1` — sync+commit+push via P29."""
    plan = plan_github_push_post29(script_dir)
    ...
    msg = message or f"chore(data-pc): P30 github push ({plan.gate_count} gates)"
    push = push_github_refresh_post28(script_dir, message=msg)
...
def validate_github_push_post29_artifact(payload: Dict[str, Any]) -> bool:
    if payload.get("status") not in ("ok", "skipped", "partial"):
        return False
    plan = payload.get("plan")
    if not isinstance(plan, dict):
        return False
    return plan.get("gate_count", 0) >= 214 and "dest_markers_ok" in plan
