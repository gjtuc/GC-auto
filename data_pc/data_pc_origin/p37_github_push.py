# -*- coding: utf-8
"""P37 — GitHub push (P36 sync → feat/data-pc-origin remote)."""
...
from data_pc_origin.gates.registry import P36_EXTENDED_ORDER
...
def plan_github_push_post36(script_dir: str) -> GithubPushPlan:
    """P36 dest markers + git branch state."""
    refresh = plan_github_refresh_post35(script_dir)
    dest = verify_dest_markers_post35(script_dir)
    ...
    return GithubPushPlan(
        ...
        gate_count=len(P36_EXTENDED_ORDER),
...
def push_github_post36(
    script_dir: str,
    *,
    message: Optional[str] = None,
) -> Dict[str, Any]:
    """`DATA_PC_GITHUB_PUSH=1` — sync+commit+push via P36."""
    ...
    msg = message or f"chore(data-pc): P37 github push ({plan.gate_count} gates)"
    push = push_github_refresh_post35(script_dir, message=msg)
...
def validate_github_push_post36_artifact(payload: Dict[str, Any]) -> bool:
    if payload.get("status") not in ("ok", "skipped", "partial"):
        return False
    plan = payload.get("plan")
    if not isinstance(plan, dict):
        return False
    return plan.get("gate_count", 0) >= 270 and "dest_markers_ok" in plan
