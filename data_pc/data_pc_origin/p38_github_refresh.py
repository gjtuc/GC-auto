# -*- coding: utf-8
"""P38 — GitHub refresh sync (P36–P37 → feat/data-pc-origin)."""
...
from data_pc_origin.gates.registry import P37_EXTENDED_ORDER
...
REFRESH_MARKER_DOCS: tuple[str, ...] = (
    "design/catalog/P36.md",
    "design/catalog/P37.md",
)
...
def plan_github_refresh_post37(script_dir: str) -> GithubRefreshPlan:
    """P23 snapshot + P36–P37 marker readiness."""
    ...
    return GithubRefreshPlan(
        ...
        gate_count=len(P37_EXTENDED_ORDER),
...
def verify_dest_markers_post37(script_dir: str) -> Dict[str, Any]:
    markers = _collect_markers(script_dir)
    return {
...
def sync_github_refresh_post37(script_dir: str, *, dry_run: bool = False) -> Dict[str, Any]:
    ...
    dest_markers = verify_dest_markers_post37(script_dir) if not dry_run else {"ok": True, "markers": []}
    return {
        "status": "ok",
        "dry_run": dry_run,
        "sync": sync,
...
def push_github_refresh_post37(
    script_dir: str,
    *,
    message: Optional[str] = None,
) -> Dict[str, Any]:
    sync = sync_github_refresh_post37(script_dir, dry_run=False)
    if sync["status"] != "ok":
        return {"status": "error", "stage": "sync", "sync": sync}

    msg = message or f"chore(data-pc): P38 github refresh ({len(P37_EXTENDED_ORDER)} gates)"
    push = push_snapshot(script_dir, message=msg)
    push["refresh"] = sync
    push["dest_markers"] = verify_dest_markers_post37(script_dir)
    return push


def validate_github_refresh_post37_artifact(payload: Dict[str, Any]) -> bool:
    if payload.get("status") not in ("ok", "skipped", "partial"):
        return False
    plan = payload.get("plan")
    if not isinstance(plan, dict):
        return False
    return plan.get("gate_count", 0) >= 278 and "markers" in plan
