...
class TestP34GithubRefresh(unittest.TestCase):
    def test_plan_real(self) -> None:
        ...
        self.assertGreaterEqual(plan.gate_count, 246)
...
class TestLiveP34GithubRefresh(unittest.TestCase):
    def test_dry_artifact(self) -> None:
        script_dir = str(Path(__file__).resolve().parents[2])
        root = Path(__file__).resolve().parents[1]
        out = run_live_p34_github_refresh(artifact_dir=root, script_dir=script_dir)
        self.assertIn(out["status"], ("ok", "partial"))
        self.assertTrue(validate_github_refresh_post33_artifact(out))
