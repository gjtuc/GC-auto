...
class TestP39GithubPush(unittest.TestCase):
    def test_plan_real(self) -> None:
        ...
        self.assertGreaterEqual(plan.gate_count, 286)
...
class TestLiveP39GithubPush(unittest.TestCase):
    def test_dry_artifact(self) -> None:
        script_dir = str(Path(__file__).resolve().parents[2])
        root = Path(__file__).resolve().parents[1]
        out = run_live_p39_github_push(artifact_dir=root, script_dir=script_dir)
        self.assertIn(out["status"], ("ok", "partial"))
        self.assertTrue(validate_github_push_post38_artifact(out))
