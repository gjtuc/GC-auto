...
- `gc_automation.py` 시작 시 `migrate_gc1_runtime_layout()` — 루트·바탕화면 `KCH`·`.cursor` 잔여 파일 자동 정리
- GC1 PC에 `Desktop\KCH` 또는 `Desktop\.cursor` 가 생기면 내용을 `_GC자동화`로 합친 뒤 빈 폴더 삭제 — **GC2/GC3 장비 PC의 KCH 폴더와 무관**
