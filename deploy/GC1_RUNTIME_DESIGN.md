...
>
> **명칭 (2026-07):** 실험실 표기는 **GC4 장비 PC** (구 GC1). 패키지·설계서 파일명 `gc1_*` / 본 문서는 **레거시 id 유지**.  
> 규칙: `docs/PC_NAMING.md` · 코드 동의어: `gc_identity.py` (`gc1`≡`gc4`).
>
...
| [PART2_L4](GC1_RUNTIME_DESIGN_PART2_L4.md) | P0~P9 action leaf 요약 |
| [PART2_L4_P0_P4](GC1_RUNTIME_DESIGN_PART2_L4_P0_P4.md) | **P0~P4 7필드 registry** (T13: 36 atoms) |
| [PART2_L4_P5_P9](GC1_RUNTIME_DESIGN_PART2_L4_P5_P9.md) | **P5~P9 7필드 + L6 + job JSON** (T14) |
| [gc_autochro_job.example.json](gc_autochro_job.example.json) | `.gc_autochro_job.json` 스키마 예시 |
...
> **JSON 예시:** [gc_autochro_job.example.json](gc_autochro_job.example.json) (T14)
...
## §L4 — 페이즈·원자 (요약 — **7필드 전량은 PART2a/b**)

> **T13:** [PART2_L4_P0_P4](GC1_RUNTIME_DESIGN_PART2_L4_P0_P4.md) — P0~P4 **36 atom** 7필드  
> **T14:** [PART2_L4_P5_P9](GC1_RUNTIME_DESIGN_PART2_L4_P5_P9.md) — P5~P9 **36 atom** + L6 + [job JSON](gc_autochro_job.example.json)  
> action sub-leaf: [PART2_L4](GC1_RUNTIME_DESIGN_PART2_L4.md) (~225)
...
**수용 파일:** [`gc1_mod_slots.json`](gc1_mod_slots.json)  
**검증:** `python scripts/validate_gc1_mod_slots.py`  
**적용 계획(dry-run):** `python scripts/apply_gc1_mod.py --dry-run` (T71)  
**상태:** `python scripts/status_gc1_mod.py` (T72)  
**완료 표시:** `python scripts/close_gc1_mod.py --mod MOD-N` (atom 패치 후)  
**입력:** `python scripts/intake_gc1_mod.py --mod MOD-N --title ... --summary ... --leaf ...` (T86)  
**일괄 점검:** `python scripts/run_gc1_mod_pipeline.py` (T87)  
**예시:** [`gc1_mod_slots.example.json`](gc1_mod_slots.example.json)  
**코드:** `mod_registry.py` (T70) · `mod_apply.py` (T71) · `mod_lifecycle.py` (T72) · `mod_intake.py` (T86)

| MOD-ID | 영향 leaf | R 변경 | 상태 |
|--------|-----------|--------|------|
| MOD-1 | (대기) | 없음 | pending — T70 atom 구현은 JSON 채운 뒤 |
| MOD-2 | (대기) | 없음 | pending |
| MOD-3 | (대기) | 없음 | pending |

채우기: `title`, `summary`, `leaf_ids` (예 `Ω.A.L4.P4.08`), `status`: `ready`
...
| Ω.A.L0 (PART1 합) | 502 | — |
| Ω.A.L1–L3 | 47 | — |
| Ω.A.L4 registry (PART2a+b) | 72 atoms | action ~225 + hancom loop |
| Ω.A.L6 (PART3) | 53 | **6×N** trim + **7×files** clean + **2×pages** parse |
| Ω.A.L7–L8 (PART4) | 45 | — |
| Ω.A.RETRY (PART6) | 32 | — |
| Ω.B (PART5) | 160 | **×mail items × attachments** |
| **합계 (典型)** | **~1174** | N=50 주입 → **+300** → **~1474** |

**가변 공식:**  
`TOTAL = 1174 + 6×N + 2×P + 7×F + 8×M×A + hancom_iters×8`  
...
- **L4 72 atom 7필드:** PART2a/b (T13–T14)  
