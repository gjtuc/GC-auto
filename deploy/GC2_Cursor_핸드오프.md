...
> **작성 시점**: 2026-06-17 (통합 repo GC2 역배포 baseline)  
> **작성 PC**: GC1 작업 후 **GC2 PC에 먼저** 역배포 (GC1 코드는 repo에 유지)
...
| 맞음 | 틀림 |
|------|------|
| **통합 repo** — GC1+GC2+GC3 코드 **공존** | GC1 모듈(`gc_autochro`, `gc_gc1` 등) **삭제** |
| zip을 **GC2 PC에** 풀어 GC2/GC3 **회귀 검증** | GC1을 GC2로 **교체**하거나 GC2 PC에 `GC_INSTANCE=gc1` |
| GC1 코드는 repo에 **그대로** → 나중에 GC1 PC에 **동일 zip** 배포 | ChemStation(8860) 로직을 GC1 PDF로 **대체** |

**「GC2 기준 역배포」** = 배포 **대상 PC가 GC2**이고, 그 PC에서 **GC2가 동작**해야 한다는 뜻. GC1 전용 코드는 **비활성 상태로 포함**된다.
...
| `deploy/gc_automation.env.gc1` | GC1 env 템플릿 (GC1 PC만, GC2 PC에 복사 금지) |
...
**ZIP 위치** (동일 파일 2곳):
```
C:\Users\User\Desktop\KCH\GC2_baseline_chemstation-gc-automation.zip
C:\Users\User\chemstation-gc-automation\deploy\GC2_baseline_chemstation-gc-automation.zip
```

**ZIP 내용** (통합 repo, ~100KB):
- Python 소스 22개 (`*.py` — `gc_architecture.py` 포함)
- bat — `gc2_make_deploy_zip.bat`, watch/force/verify, GC1 setup 스크립트 포함
- `requirements.txt`
- `test_gc_sanitize.py`, `test_gc_force_auth.py`
- `.env.example`
- `.cursor/rules/gc-initiation-force.mdc`
- `deploy/` — `gc_automation.env.gc1`, `gc_automation.env.gc2`, `make_gc2_baseline_zip.ps1`, `GC2_Cursor_핸드오프.md`
...
# 2) zip 압축 해제 (임시 폴더)
$tmp = "$env:TEMP\gc2_baseline_extract"
Expand-Archive -Path "C:\Users\User\chemstation-gc-automation\deploy\GC2_baseline_chemstation-gc-automation.zip" -DestinationPath $tmp -Force
...
# deploy\gc_automation.env (로컬 비밀번호 원본)는 zip에 없음 — .gc1 / .gc2 템플릿만
```
...
| GC1 | `Desktop\박은규\gc_automation.env` | `deploy\gc_automation.env.gc1` 템플릿 + 실제 비밀번호 |
...
```powershell
# GC1 PC에서
Expand-Archive -Path "<zip경로>\GC2_baseline_chemstation-gc-automation.zip" -DestinationPath "$env:TEMP\gc1_deploy" -Force
...
Copy-Item "C:\Users\User\chemstation-gc-automation\deploy\gc_automation.env.gc1" "C:\Users\User\Desktop\박은규\gc_automation.env" -Force
...
2. **통합 baseline zip 관리** — `deploy\GC2_baseline_chemstation-gc-automation.zip` (GC1+GC2 코드 포함). GC2 merge 후 **동일 zip**을 GC1 PC에 forward deploy
...
*이 문서와 `GC2_baseline_chemstation-gc-automation.zip`을 함께 GC2 Cursor에 전달하세요.*
