...
> **작업 PC:** GC1 장비 PC에서 RT·표준가스 실측 → repo 반영 → 데이터 PC에서 `--no-archive` 검증  
...
데이터 PC(또는 `Desktop\.cursor\`)에서 G:/Origin 없이:
...
다른 PC(차헌 GC2, 데이터 PC)는 **pull 후** 작업.
...
- [ ] 7.5 GC1 KCH xlsx로 RT 실측 검증 (**GC1 PC — 사용자**)
- [ ] 7.6 표준가스로 CALIB 입력 + `GC1_CALIB_READY = True` (**사용자**)
- [ ] 7.7 `--no-archive` 계산 검증 (**데이터 PC**)

---

## 다음: Step 8

[`deploy/STEP8_e2e.md`](STEP8_e2e.md) — GC1 메일 → 데이터 PC → G: → Origin (Step 7 CALIB 완료 후)
