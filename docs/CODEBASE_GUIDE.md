...
## 1. 연구실 자동화는 「장비 PC」와 「데이터 PC」

> PC 명칭: [`PC_NAMING.md`](PC_NAMING.md)

| 종류 | 누가 | PC 이름 | 스크립트 |
|------|------|---------|----------|
| **장비 PC** | 은규 (GC1) | GC1 장비 PC | `gc_automation.py` |
| **장비 PC** | 차헌 (GC2/3) | GC2/GC3 장비 PC | `gc_automation.py` |
| **데이터 PC** | 은규 | **은규 PC** | `data_pc/촉매 반응 계산.py` |
| **데이터 PC** | 차헌 | **차헌 PC** | `data_pc/촉매 반응 계산.py` |

**장비 PC** = KCH **원본** 엑셀 만들고 **메일 발송** (수신: 본인 데이터 PC)  
**은규 PC / 차헌 PC** = 메일 **수신** → 수율/전환율 → **G:** → **Origin**
