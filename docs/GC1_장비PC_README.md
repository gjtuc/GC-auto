...
수율/전환율 계산, G: 드라이브 정리, Origin 업데이트는 **데이터 PC-은규**에서 합니다.
...
4. 네이버 SMTP로 **데이터 PC-은규 메일**에 첨부 발송
...
| `TARGET_EMAIL` (약 70행) | **은규 데이터 PC 네이버 메일** (현재 차헌 주소로 되어 있음) |
...
3. `02_데이터PC_은규/` 내용을 **은규 데이터 PC**로 옮겨 2~4단계 세팅
