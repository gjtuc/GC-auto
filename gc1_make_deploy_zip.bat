...
set "DESKTOP_GC=%USERPROFILE%\Desktop\박은규\_GC자동화"
if not exist "%DESKTOP_GC%" mkdir "%DESKTOP_GC%"
copy /y "%DEST%" "%DESKTOP_GC%\GC1_forward_chemstation-gc-automation.zip" >nul
copy /y "%HANDOFF%" "%DESKTOP_GC%\GC1_Cursor_핸드오프.md" >nul

echo.
echo  [완료] %DEST%
echo  [복사] %DESKTOP_GC%\GC1_forward_chemstation-gc-automation.zip
echo  [복사] %DESKTOP_GC%\GC1_Cursor_핸드오프.md
echo.
echo  GC1 장비 PC에서:
