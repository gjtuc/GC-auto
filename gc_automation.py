...
=============================================================================
  ...
  GC2/GC3: 핫스pot 연결 세션당 엑셀·메일 1회 (GC1과 동일, 슬롯 한도 없음)
  GC1: 핫스pot **세션당** PDF·엑셀·메일 1회
...
from gc_instance import ensure_watch_exclusive
...
def main() -> None:
    ...
    if args.watch:
        lock_result = ensure_watch_exclusive(
            config.excel_output_dir, HEARTBEAT_TOLERANCE_MINUTES
        )
        if lock_result == "already_healthy":
            print("[안내] GC 감시가 이미 정상 실행 중 — 중복 프로세스 자동 종료")
            return
        if lock_result == "failed":
            print("[오류] GC 감시 lock 획득 실패 — gc_stop_watch.bat 후 다시 시작하세요.")
            return
